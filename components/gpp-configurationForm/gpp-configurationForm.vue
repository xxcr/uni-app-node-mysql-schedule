		...
		<view class="gpp-cf-form" :class="{'bottomShadow':isCard}" v-for="(item, index) in formTemplate" :key="index">
			...
			<view class="gpp-cf-content" :class="{'hidden':showItemIds.indexOf(item.id)==-1}">
				<view v-for="(detailItem, detailIndex) in item.object" :key="detailIndex">
						...
						<view class="item-content">
							<view v-if="type=='fill'">
								<picker mode="date" @change="datePickerChange($event, detailItem.textName)">
									<view class="item-select" v-if="itemValue[detailItem.textName]==''
									 ?itemValue[detailItem.textName]= date :itemValue[detailItem.textName]= date">{{itemValue[detailItem.textName]}}</view>
					...
					<!-- startTime -->
					<view class="gpp-cf-content-item" v-if="detailItem.controlType == 'startTime'">
						<view class="item-name"><text class="red" v-if="detailItem.isMustfill">*</text>{{detailItem.subject}}</view>
						<view class="item-content">
							<view v-if="type=='fill'">
								<picker mode="time" :value="time.start" @change="timePickerChange($event, detailItem.textName)">
									<view class="item-select" v-if="itemValue[detailItem.textName]==''
									 ?itemValue[detailItem.textName]= time.start :1">{{itemValue[detailItem.textName]}}</view>
									<text class="item-ct-unit arrowBottom" v-if="itemValue[detailItem.textName]==''"></text>
									<text class="item-ct-unit fork" @click.stop="itemValue[detailItem.textName]=''" v-else></text>
								</picker>
							</view>
					...
					<!-- endTime -->
					<view class="gpp-cf-content-item" v-if="detailItem.controlType == 'endTime'">
						<view class="item-name"><text class="red" v-if="detailItem.isMustfill">*</text>{{detailItem.subject}}</view>
						<view class="item-content">
							<view v-if="type=='fill'">
								<picker mode="time" :value="time.end" @change="timePickerChange($event, detailItem.textName)">
									<view class="item-select" v-if="itemValue[detailItem.textName]==''
									 ?itemValue[detailItem.textName]= time.end :1">{{itemValue[detailItem.textName]}}</view>
									<text class="item-ct-unit arrowBottom" v-if="itemValue[detailItem.textName]==''"></text>
									<text class="item-ct-unit fork" @click.stop="itemValue[detailItem.textName]=''" v-else></text>
								</picker>
							</view>
